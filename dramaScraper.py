...
class Drama:
    async def request(self, url, headers: dict = {}, data: dict = {}, get="text", method="get"):
        ...
        headers['Cookie']='ci_session=lb7b4q0elgc59l021q05d4ba1se3s4mp'
        if method=="post":
    ...
    async def get_title_links(self, url):
        ...
        links = [link.get("data-video") if link.get("data-video").startswith("http")
                 else f"https:{link.get('data-video')}" for link in soup.find("div", {"class": "anime_muti_link"}).find("ul").find_all("li")]
