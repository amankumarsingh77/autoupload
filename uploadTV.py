...
import datetime
_base_url = "https://v15b.dramaworldapp.xyz"
            ...
            if int(season["season_number"]) != 0:
                if int(season["season_number"]) != 1:
                    year = datetime.datetime.strptime(
                        str(season["air_date"]), "%Y-%m-%d").year
                    name = meta["name"]
...
async def upload_all_serie():
    for char in string.ascii_uppercase:
        ...
        dramas = ["https://watchasian.sh"+re.search(r"/[/\-\.\w\d]*", drama.get("onclick")).group(
        ) for drama in soup.find("ul", {"class": "list-episode-item"}).find_all("h3", {"class": "title"})]
        for drama in dramas:
